...
copy_table() {
    ...
    local source_count=$(mysql -h localhost -P "$PROD_PORT" -u paul -p -sN -e "SELECT COUNT(*) FROM whatsfresh.$table_name" 2>/dev/null || echo "0")
    ...
    local temp_dump="/tmp/${table_name}_dump_$$.sql"
    ...
    # Step 4: Verify copy
    local target_count=$(mysql -h localhost -P "$TEST_PORT" -u root -p -sN -e "SELECT COUNT(*) FROM whatsfresh.$table_name" 2>/dev/null || echo "0")
